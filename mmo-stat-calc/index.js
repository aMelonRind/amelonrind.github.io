...
function calculateStats() {
  ...
  const basePhysical = parseFloat(basePhysicalElement.value) || 0
  const baseMagic = parseFloat(baseMagicElement.value) || 0
  const baseCritChance = parseFloat(baseCritChanceElement.value) || 0
  const baseCritDmg = parseFloat(baseCritDmgElement.value) || 0
  const gemSlots = Math.floor(parseFloat(gemSlotsElement.value) || 0)
  ...
  window.localStorage.setItem('inputCache', JSON.stringify([damageType, basePhysical, baseMagic, baseCritChance, baseCritDmg, gemSlots, critOnly]))

  const res = calcStats(damageType, basePhysical, baseMagic, baseCritChance, baseCritDmg, gemSlots, critOnly, 64).map(v => v.name)
