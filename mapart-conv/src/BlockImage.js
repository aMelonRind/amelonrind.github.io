...
class BlockImage {
  ...
  async toLitematicSeparated() {
    const palette = BlockPalette.fromRebaneUrlInput()
    const layers = this.#buildPalettedBlocks(palette)
    const { map } = palette.optimize(layers)
    ...
    const regions = {}
    const ids = palette.base.map((state, i) => map[i] ? BlockState.getId(state) : null)
    for (const id of new Set(ids)) {
      if (!id) continue
      const regionBase = ['air', ...palette.base.filter(state => BlockState.getId(state) === id).sort()]
      const map2 = Uint16Array.from(palette.base, state => ((regionBase.indexOf(state) + 1) || 1) - 1)

      const bits = BigInt(Math.max(2, 32 - Math.clz32(regionBase.length - 1)))
      const longArr = new BigUint64Array(Math.ceil(volume * Number(bits) / 64))
      const bitMask = (1 << Number(bits)) - 1
      const mask64 = (1n << 64n) - 1n
      let index = 0
      let buf = 0n
      let bufs = 0n
      const bigIntMap = BigUint64Array.from(map2, v => BigInt(v & bitMask))
      layers.forEach(layer => layer.forEach(v => {
        if (map2[v]) {
          totalBlocks++
          buf |= bigIntMap[v] << bufs
      ...
      regions[id] = {
        ...regionCommon,
        BlockStatePalette: regionBase.map(state => BlockState.from(state).toNbt()),
        BlockStates: longArr
      }
    }
    ...
    const nbt = new NBT.NBTData({
      ...
      Metadata: {
        ...
        Author: this.author,
        Description: this.description ?? 'Converted on https://amelonrind.github.io/mapart-conv',
        RegionCount: new NBT.Int32(Object.keys(regions).length),
        TotalVolume: new NBT.Int32(volume),
        TotalBlocks: new NBT.Int32(totalBlocks),
        TimeCreated: this.timeCreated,
        TimeModified: BigInt(Date.now()),
