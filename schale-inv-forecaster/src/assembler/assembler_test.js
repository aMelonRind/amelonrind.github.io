...
export async function bulkTest() {
  ...
  for (const [key, res] of results.entries()) {
    const arr = key.split(',')
    const board = BigInt(arr.shift() ?? '0')
    origRes[`${board},${arr.join('')}`] = res.total
    total++
  }
  let pass = 0
  for (const [key, total] of Object.entries(origRes)) {
    const [board, items] = key.split(',')
    await generateTest(items, BigInt(board))
    if (results.get(`${board},${items.split('').join(',')}`)?.total === total) {
      pass++
    }
  }
  console.log(`bulk test total: ${total}, passed: ${pass}`)
}
