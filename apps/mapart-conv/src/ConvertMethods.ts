import BlockImage, { BlockPalette } from "./BlockImage.ts";
import Form, { t } from "./Form.tsx";
import MainContext from "./MainContext.ts";
import { ITask } from "./TaskManager.ts";
import { downloadBlob, isDev, requireNonNull } from "../../../lib/util.ts";
import rebane2dViewExampleUrl from "../img/rebane2dViewExample.png?url";
import { TranslationKey } from "../i18n.ts";
import { Accessor } from "solid-js";

export default class ConvertMethod {
  static readonly methods: Map<string, ConvertMethod> = new Map()
  readonly name
  ...
  static async run(name: string, task: ITask = ITask.DUMMY, ctx: MainContext | null = MainContext.getCurrent()) {
  ...
  constructor ({ name, displayName, canRun = ctx => ctx != null, action }: {
      name: string;
      displayName: Accessor<string>;
      canRun?: (this: ConvertMethod, ctx: MainContext | null) => any;
      action: (this: ConvertMethod, task: ITask, ctx: MainContext | null) => Promise<any>;
    }) {
   ...
   * all 0..255
   */
  getNearest(r: number, g: number, b: number, a: number) {
...
new ConvertMethod({
  name: 'nearest',
  displayName: () => t('control.convert.method.nearest.name'),
  canRun: ctx => ctx && !ctx.base.isBlock(),
  async action(task, ctx) {
    if (!ctx) return
    await Form.send('nearest', {}, {
      title: () => t('control.convert.method.nearest.title'),
      description: () => t('control.convert.method.nearest.description')
    })
...
if (isDev())
new ConvertMethod({
  name: '[dev] rebane 2d palette hash gen', // generate hashes for restoring 2d view
  displayName: () => '[dev] rebane 2d palette hash gen',
  ...
  async action(task, ctx) {
    ...
    const desc = `${img.width}x${img.height} (${w.toFixed(2)}x${h.toFixed(2)})`
    ...
    const hashSamples: Uint32Array[] = new Array(h + 1)
    { // full black hash
      let hash = 5
    ...
    const sorted = hashSamples
      .flatMap((arr, y) => Array.from(arr, (hash): [hash: number, y: number] => [hash, y]))
      .sort((a, b) => a[0] - b[0])
...
{
  const loader = import("./data/rebane2dHashes.json").then(({default: json}) => ({
  ...
  new ConvertMethod({
    ...
    displayName: () => t('control.convert.method.rebane_2d_restore.name'),
    ...
    async action(task, ctx) {
      ...
      await Form.send('rebane2dRestore', {}, {
        title: () => t('control.convert.method.rebane_2d_restore.title'),
        description: () => t('control.convert.method.rebane_2d_restore.description'),
        image: rebane2dViewExampleUrl
      })
      ...
      if (!Number.isInteger(w) || !Number.isInteger(h)) {
        Form.send('rebane2dRestore.33', {}, {
          title: () => t('control.convert.method.rebane_2d_restore.33.title'),
          description: () => t('control.convert.method.rebane_2d_restore.33.description', {
            imgWidth: img.width,
            imgHeight: img.height,
            mapWidth: w.toFixed(2),
            mapHeight: h.toFixed(2)
          }),
          noCancel: true
        })
        return
      ...
      if (unk) {
        Form.send('rebane2dRestore.nfr', {}, {
          title: () => t('control.convert.method.rebane_2d_restore.not_fully_restored.title'),
          description: () => t('control.convert.method.rebane_2d_restore.not_fully_restored.description', {
            missing: unk,
            size: w * h,
            percentage: (unk / (w * h) * 100).toFixed(2)
          }),
          noCancel: true
        })
      }
