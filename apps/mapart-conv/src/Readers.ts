...
import { firstValue } from "../../../lib/util.ts";
import JSZip from "jszip";
import { LitematicNbt, MapDatNbt, PaletteNbt, SchematicNbt, StructureNbt } from "../../types.ts";

export default class Readers {

  static async readItems(items: DataTransferItemList | null | undefined, task: ITask): Promise<BaseImage | null> {
    if (!items) return null
    for (const item of items) {
      if (item.type === 'text/plain') { // urls
        return new Promise(res => {
          item.getAsString(async str => res(
            str.startsWith('https://rebane2001.com/mapartcraft/?preset=')
            ? null
            : await ImageReaders.readURL(str)
          ))
        })
  ...
  static async readStructure(
    reader: () => Promise<NBT.NBTData>,
    task: ITask = ITask.DUMMY,
    filename: string | null = null
  ): Promise<BlockImage> {
    await task.push('Reading as structure', 3)
    await task.force().swap('Reading nbt')
    const root: StructureNbt = await reader() as any
  ...
  static async readSchematic(
    ...
    await task.force().swap('Reading nbt')
    const root: SchematicNbt = await reader() as any
  ...
  static async readMapDat(
    ...
    await task.force().swap('Reading nbt')
    const root: MapDatNbt = await reader() as any
  ...
  static async readLitematic(
    ...
    await task.force().swap('Reading nbt')
    const root: LitematicNbt = await reader() as any
  ...
  static async readZip(file: File, task: ITask = ITask.DUMMY): Promise<BlockImage> {
    ...
    const data: { [name: string]: BlockImage; } = {}
    const promises = []
    let minX = Infinity
    let minY = 0
    let maxX = 0
    ...
    for (const [name, pref, x, y, type] of matches) {
      ...
      if (ny < minY) minY = ny
      if (ny > maxY) maxY = ny
      promises.push(new Promise<BlockImage>(async (res, rej) => {
...
export class ImageReaders {
  static readFile(file: File): Promise<RGBAImage> {
  ...
  static readURL(
    str: string,
    name = str.startsWith('data:') ? null : str.match(/(?<=[\\/])[^\\/]+?(?:\.\w+)?$/)?.[1] ?? null
  ): Promise<RGBAImage> {
    return new Promise((res, rej) => {
      const img = new Image()
...
export class BlockImageBuilder {
  static readonly WATER_MASK = (1 << 10) - 1 // deepest water color
  static map: Record<string, number | { if: string, is: string, then: number, else: number }> = block2color
  static pre13map: { [key: number]: number } = pre13color // index = data_id << 8 | block_id
  readonly blocks: Uint8Array
  readonly heights: Int16Array
  readonly waters: Int16Array // bitfield map, 1 for top, 2 for second top, 4 for third...
  readonly width: number
  readonly height: number
  #hasTransparent = false

  static readMcPalette(palette: PaletteNbt): Uint8Array {
  ...
  static readSchematicIndexes(blockIds: Int8Array, dataIds: Int8Array): Uint8Array {
  ...
  putPos(x: number, y: number, z: number, color: number) {
    if (!color) {
  ...
  putIndex(index: number, y: number, color: number) {
    if (!color) {
      this.#hasTransparent = true
      return // transparent (not air)
    }
  ...
  #putInternal(index: number, y: number, color: number) {
    if (color !== 12) {
  ...
  async build(): Promise<BlockImage> {
    const heightSet = new Set(this.heights)
    const isFlat = heightSet.size === 1
    const topY = isFlat ? firstValue(heightSet) : -32000
    const getSlope = (index: number) => Math.sign(this.heights[index] - (this.heights[index - this.width] ?? topY)) + 1
    ...
    if (waterSet.size) {
      ...
      if (isStair) {
        ...
        const isShallow: (i: number) => boolean | number = hasTopRow
